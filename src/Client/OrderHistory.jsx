// components/DispatchTimeline.jsx
...
const OrderHistory = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        // Filter for clients - only show their company's dispatches
        if (!isAdmin && userCompany) {
          const normalizedUserCompany = userCompany.toLowerCase().trim();
          formatted = formatted.filter((item) => {
            const normalizedItemCompany = item.companyName
              ?.toLowerCase()
              .trim();
  ...
  const handleEdit = async (row) => {
    ...
    try {
      ...
      const res = await axios.get(
        `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/dispatch-orders/${row.dispatchId}`,
        { withCredentials: true },
      );
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/dispatch-orders/${editingDispatch.dispatchId}`,
        formData,
        {
          withCredentials: true,
  ...
  const handleDelete = async (row) => {
    ...
    try {
      ...
      setData((prev) =>
        prev.filter((item) => item.dispatchId !== row.dispatchId),
      );
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-${config.color}-50 text-${config.color}-700`}
      >
              ...
              <tbody>
                {isLoading ? (
                  ...
                  paginatedData.map((row, index) => (
                    <tr
                      key={
                        row.dispatchId || `${row.poId}-${row.itemId}-${index}`
                      }
                    ...
                    >
                      ...
                      <td className="border border-gray-300 px-3 py-2 align-middle">
                        {row.batchNumber ? (
                          <span className="inline-flex px-2 py-1 text-xs font-medium bg-amber-50 text-amber-700 rounded-md border border-amber-200">
                            {row.batchNumber}
                          </span>
                        ) : (
